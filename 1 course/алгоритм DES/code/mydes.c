...
uint64_t first_permutation(uint64_t input)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        init_perm_res <<= 1; // присваивание каждому биту 0 или 1 в соответствии с таблицей IP
...
void generate_keys(uint64_t key, uint64_t sub_key[16])
{
    ...
    for (i = 0; i < 56; i++)
    {
        C0D0table <<= 1; // присваивание i биту C0D0table значение бита key под номером C0D0[i]
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        {
            sub_key[i] <<= 1; // присваивание каждому биту 0 или 1 в соответствии с таблицей Pл
...
void Feistel(uint32_t *L, uint32_t *R, uint64_t sub_key[16], int i, char mode)
{
    ...
    // операция R XOR K
    if (mode == 'd')
        s_input = s_input ^ sub_key[i]; // если режим шифрования
    else
        s_input = s_input ^ sub_key[15 - i]; // если режим дешифрования
    // S таблицы замен
    for (int j = 0; j < 8; j++)
    {
        ...
        s_output <<= 4; // занесение 4 бит из таблицы в конечный вектор
        s_output |= (uint32_t)(S[j][16 * row + column] & 0x0f);
    }
    uint32_t f_function_res = 0;
    for (int j = 0; j < 32; j++)
    {
        f_function_res <<= 1; // перестановка P
        f_function_res |= (s_output >> (32 - P[j])) & 0x00000001;
    }
    uint32_t temp = (*R);
    (*R) = (*L) ^ f_function_res; // занесение L XOR результат перестановки в R
    (*L) = temp;                  // занесение R в L
}
...
uint64_t last_permutation(uint64_t pre_output)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        inv_init_perm_res <<= 1; // последовательное присваивание каждому биту 0 или 1
...
// 56 бита ключа и режим,  'e' = шифрация; 'd' = дешифрация
uint64_t des(uint64_t input, uint64_t key, char mode)
{
    ...
    input = first_permutation(input);                 // начальная перестановка
    L = (uint32_t)(input >> 32) & 0x00000000ffffffff; // разбиение на два вектора
    R = (uint32_t)input & 0x00000000ffffffff;
    generate_keys(key, sub_key); // генерация всех раундовых ключей
    for (i = 0; i < 16; i++)
        Feistel(&L, &R, sub_key, i, mode); // 16 раундов сети Фейстеля
    output = (((uint64_t)R) << 32) | (uint64_t)L;
    output = last_permutation(output); // конечная перестановка
    return output;
}
...
// перенос бинарного кода в текст
void bin_to_text(uint64_t hexstr, char *text)
{
    int temp = 0;
    for (int i = 0; i < 8; i++)
    {
...
void encryption()
{
    ...
    uint64_t input = get_text(8); // ввод текста для шифрования
    printf("Enter key for encryption\n");
    uint64_t key = get_text(7);             // ввод ключа в текстовом формате
    uint64_t result = des(input, key, 'e'); // шифрование
    printf("Encrypted: %16llx\n", result);  // вывод зашифрованного текста в HEX
}
// функция дешифрования
void decryption()
{
    printf("Enter text for decryption (HEX), max 16 symbols\n");
    uint64_t input;
    scanf("%llx", &input); // ввод текста для дешифрования в HEX
    printf("Enter key for decryption\n");
    uint64_t key = get_text(7); // ввод ключа в текстовом формате
    uint64_t result = des(input, key, 'd');
    printf("Decrypted: %016llx\n", result); // вывод дешифрованного текста в HEX
    char text[9];
    bin_to_text(result, text);
    printf("Text: %s\n", text); // вывод дешифрованного текста
}

int main(int argc, const char *argv[])
{
    char mode; // выбор режима работы программы
    while (1)
    {
        printf("Choose program mode\nEnter ''e'' for encryption\nEnter ''d'' for decryption\nEnter ''0'' for exit\n");
        do
            scanf("%c", &mode);
        while (mode != 'e' && mode != 'd' && mode != '0');
        if (mode == 'e')
            encryption();
        else if (mode == 'd')
            decryption();
